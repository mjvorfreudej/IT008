...
namespace Bai05
{
    public partial class MainForm : Form
    {
        ...
        private void LoadButtonIcon()
        {
            try
            {
                // Đường dẫn tương đối đến thư mục project
                string projectPath = Path.GetDirectoryName(Application.StartupPath);
                projectPath = Path.GetDirectoryName(projectPath);
                string iconPath = Path.Combine(projectPath, "Resources", "add_icon.png");
                
                // Thử load từ thư mục project
                ...
                // Thử load từ thư mục output
                iconPath = Path.Combine(Application.StartupPath, "Resources", "add_icon.png");
                if (File.Exists(iconPath))
                {
                    Image img = Image.FromFile(iconPath);
                    btnThemMoi.Image = new Bitmap(img, new Size(20, 20));
